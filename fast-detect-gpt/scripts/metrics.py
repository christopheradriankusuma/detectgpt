...
def get_precision_recall_metrics(real_preds, sample_preds):
    precision, recall, thresholds = precision_recall_curve([0] * len(real_preds) + [1] * len(sample_preds),
                                                  real_preds + sample_preds)
    pr_auc = auc(recall, precision)
    return precision.tolist(), recall.tolist(), float(pr_auc), thresholds.tolist()
