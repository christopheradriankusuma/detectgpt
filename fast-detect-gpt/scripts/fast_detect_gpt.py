...
from model import load_tokenizer, load_model, get_reference_model_name
...
def experiment(args):
    ...
    if args.reference_model_name != args.scoring_model_name:
        reference_model_name = get_reference_model_name(args.scoring_model_name, args.reference_model_name)
        p, d, g, _, s = re.compile(r'(.+)/(\w+)_([\w\d.-]+)\.([a-z][\w\d.-]+)_([\w\d.-]+)').findall(args.output_file)[0]
        args.output_file = f'{p}/{d}_{g}.{reference_model_name}_{s}'
        reference_tokenizer = load_tokenizer(reference_model_name, args.dataset, args.cache_dir)
        reference_model = load_model(reference_model_name, args.device, args.cache_dir)
    ...
    fpr, tpr, roc_auc, roc_thresholds = get_roc_metrics(predictions['real'], predictions['samples'])
    p, r, pr_auc, pr_thresholds = get_precision_recall_metrics(predictions['real'], predictions['samples'])
    ...
    results = { 'name': f'{name}_threshold',
                ...
                'raw_results': results,
                'metrics': {'roc_auc': roc_auc, 'fpr': fpr, 'tpr': tpr, 'thresholds': roc_thresholds},
                'pr_metrics': {'pr_auc': pr_auc, 'precision': p, 'recall': r, 'thresholds': pr_thresholds},
                'loss': 1 - pr_auc}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cache_dir', type=str, default="/home/a/adrian/cache")
